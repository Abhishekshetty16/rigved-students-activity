...
app.get('/users',(request,response) =>{
    mongoClient.connect(dbURL,{useNewUrlParser:true},(error,client)=>{
        ...
        let cursor=db.collection("sports").find();
        let users=[];
...
app.post("/users",(request,response)=>{
    ...
    mongoClient.connect(dbURL,{useNewUrlParser:true},(error,client)=>{
        if(error) throw error;
        let db =client.db("mydb");
        db.collection("sports").insertOne(userDocument,(err,res)=>{
            if(err){
...
app.get("/users/:id",(request,response)=>{
    ...
    mongoClient.connect(dbURL,{useNewUrlParser:true},(error,client)=>{
        if(error) throw error;
        let db=client.db("mydb");
        db.collection("sports").findOne({_id:id})
        .then((doc)=>{
            if(doc !=null){
                response.json(doc);
            }else{
...
app.delete("/users/:id",(request,response)=>{
    ...
    mongoClient.connect(dbURL,{useNewUrlParser:true},(error,client)=>{
        if(error) throw error;
        let db=client.db("mydb");
        db.collection("sports").deleteOne({_id:id})
        .then((doc)=>{
            response.json(doc);
            client.close();
        });
...
app.put("/users/:id/:age",(request,response)=>{
    ...
    mongoClient.connect(dbURL,{useNewUrlParser:true},(error,client)=>{
        if(error) throw error;
        let db=client.db("mydb");
        db.collection("sports").updateOne({_id:id},{$set:{age:ageNew}})
        .then((doc)=>{
            response.json(doc);
            client.close();
        });
